...
class SingleDeltaEnv(BaseEnv):
    ...
    def _get_reward(self, state, info) -> float:
        ...
        # idea 1: double reward for grasping "close" to object, o/w double reward for not grasping
        # idea: reward opening grasp over basket position
        #       o/w reward
        # idea 2: penalise distance from closest cube and reward distance from other robot arm (see Desmos)
        # idea 3: encourage smoothness of the movement --> subtract L2 norm of action, consider angle with old action
        # weighted sum of terms: 50% distance potential, 30% grasping, 10% smoothness, 10% basket
        # log reward components for baseline policy to design
    ...
    def step(
        ...
        action_arm0 = self.ik_policy0.act() * 0.0
        action_arm1 = self.ik_policy1.act() + self._process_action(action)
