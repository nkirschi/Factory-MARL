# SOURCE:
# https://github.com/openai/gym/blob/master/gym/envs/mujoco/mujoco_rendering.py
...
class BaseRender:
    ...
    def render_score(self, score0: int, score1: int):
        mujoco.mjr_overlay(
...
class OffScreenViewer(BaseRender):
    ...
    def __init__(
        ...
        print("Creating Backend: ", self.backend)
    ...
    def render(
        ...
        if scores is not None:
            self.render_score(scores[0], scores[1])
    ...
    def render(
        ...
        if render_mode in ["rgb_array", "depth_array"]:
            return viewer.render(
                render_mode=render_mode, camera_id=self.camera_id, scores=scores
            )
        elif render_mode == "human":
            return viewer.render(scores=scores)
